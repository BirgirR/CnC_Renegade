...
//	The resolution used when nothing has been stored in the registry yet. The
//	original 800x600 was a safe pick for 2002 hardware; it is far below what any
//	current display runs at.
//
//	This is only the starting point. If the adapter cannot set this mode,
//	Registry_Load_Render_Device walks its own ladder of smaller resolutions
//	(2048x1536, 1920x1440, 1600x1200, 1280x1024, 1024x768, 800x600, 640x480)
//	until one is accepted, so an unusual display still gets a working mode.
const int DEFAULT_RESOLUTION_WIDTH = 2560;
const int DEFAULT_RESOLUTION_HEIGHT = 1440;
...
{
	WWDEBUG_SAY(("D3D call failed at %s(%d): ", file, line));
...
unsigned DX8Wrapper::CurrentBaseVertexIndex = 0;

void DX8Wrapper::Set_DX8_Z_Bias(unsigned level)
{
	// See the declaration: D3D8's integer ZBIAS level maps onto D3D9's
	// float D3DRS_DEPTHBIAS. The scale is empirical.
	const float DX8_ZBIAS_SCALE = 0.00005f;
	float bias = -((float)level) * DX8_ZBIAS_SCALE;
...
bool DX8Wrapper::Init(void * hwnd, bool lite)
{
	...
	*/
	...
	if (!lite) {
		// The renderer targets D3D9 now; load the matching runtime.
...
bool DX8Wrapper::Create_Device(void)
{
	...
	if (FAILED( D3DInterface->GetAdapterIdentifier(CurRenderDevice,0 /* D3DENUM_NO_WHQL_LEVEL: removed in D3D9 */,&CurrentAdapterIdentifier))) {
		return false;
	}
...
void DX8Wrapper::Enumerate_Devices()
{
	...
	for (int adapter_index=0; adapter_index<adapter_count; adapter_index++) {
		...
		HRESULT res = D3DInterface->GetAdapterIdentifier(adapter_index,0 /* D3DENUM_NO_WHQL_LEVEL: removed in D3D9 */,&id);
		...
		if (res == D3D_OK) {
			...
			*/
			...
			D3DInterface->GetAdapterIdentifier(adapter_index,0 /* D3DENUM_NO_WHQL_LEVEL: removed in D3D9 */,&desc.AdapterIdentifier);
			...
			*/
			...
			for (int mode_index=0; mode_index<mode_count; mode_index++) {
				...
				HRESULT res_inner = D3DInterface->EnumAdapterModes(adapter_index, D3DFMT_X8R8G8B8, mode_index, &d3dmode);
...
bool DX8Wrapper::Set_Render_Device(int dev, int width, int height, int bits, int windowed, bool resize_window)
{
	...
	*/
	...
	WWDEBUG_SAY(("Attempting Set_Render_Device: name: %s, width: %d, height: %d, windowed: %d\r\n",
		(const char *)_RenderDeviceNameTable[CurRenderDevice],ResolutionWidth,ResolutionHeight,(IsWindowed ? 1 : 0)));
	...
	unsigned short vertex_count)
{
	...
	// D3D9's SetStreamSource takes a byte offset before the stride, and FVF codes
	// moved from SetVertexShader to SetFVF.
	...
	// D3D9 removed SetIndices' BaseVertexIndex parameter; it is passed to
	// DrawIndexedPrimitive instead, so remember it here.
	DX8CALL(SetIndices(
		static_cast<DX8IndexBufferClass*>(dyn_ib_access.IndexBuffer)->Get_DX8_Index_Buffer()));
	CurrentBaseVertexIndex = dyn_vb_access.VertexBufferOffset;
	DX8_RECORD_INDEX_BUFFER_CHANGE();

	DX8CALL(DrawIndexedPrimitive(
		D3DPT_TRIANGLELIST,
		CurrentBaseVertexIndex,
		0,		// start vertex
		vertex_count,
...
void DX8Wrapper::Apply_Render_State_Changes()
{
	...
	for (unsigned i=0;i<MAX_TEXTURE_STAGES;++i,mask<<=1) {
		if (render_state_changed&mask) {
			SNAPSHOT_SAY(("DX8 - apply texture %d (%s)\n",i,(const char *)(render_state.Textures[i] ? render_state.Textures[i]->Get_Full_Path() : "NULL")));
	...
	{
		unsigned mask_inner=LIGHT0_CHANGED;
		for (unsigned index=0;index<4;++index,mask_inner<<=1) {
			if (render_state_changed&mask_inner) {
	...
	if (render_state_changed&INDEX_BUFFER_CHANGED) {
		...
		if (render_state.index_buffer) {
			...
			case BUFFER_TYPE_DX8:
			case BUFFER_TYPE_DYNAMIC_DX8:
				// BaseVertexIndex moved to DrawIndexedPrimitive in D3D9.
				DX8CALL(SetIndices(
					static_cast<DX8IndexBufferClass*>(render_state.index_buffer)->Get_DX8_Index_Buffer()));
				CurrentBaseVertexIndex = render_state.index_base_offset+render_state.vba_offset;
				DX8_RECORD_INDEX_BUFFER_CHANGE();
				break;
			case BUFFER_TYPE_SORTING:
			case BUFFER_TYPE_DYNAMIC_SORTING:
...
IDirect3DSurface9 * DX8Wrapper::_Create_DX8_Surface(unsigned int width, unsigned int height, WW3DFormat format)
{
	...
	// D3D9 renamed CreateImageSurface and added pool + shared-handle parameters.
	DX8CALL(CreateOffscreenPlainSurface(width, height, WW3DFormat_To_D3DFormat(format),
...
IDirect3DSurface9 * DX8Wrapper::_Get_DX8_Front_Buffer()
{
	...
	DX8CALL(CreateOffscreenPlainSurface(mode.Width,mode.Height,D3DFMT_A8R8G8B8,
													 D3DPOOL_SYSTEMMEM,&fb,NULL));

	// D3D9 renamed this and added a swap-chain index.
...
{
	DX8_Assert();
	// D3D9 replaced the byte-budgeted call with an evict-everything one.
	(void)bytes;
...
const char* DX8Wrapper::Get_DX8_Texture_Stage_State_Name(unsigned state)
{
	switch (state) {
