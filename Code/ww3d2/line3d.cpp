...
void Line3DClass::Render(RenderInfoClass & rinfo)
{
	...
	DynamicVBAccessClass vb(BUFFER_TYPE_DYNAMIC_DX8,dynamic_fvf_type,8);
	{
		...
		unsigned char *vb_inner=(unsigned char*)Lock.Get_Formatted_Vertex_Array();
		int i;
		unsigned int color=DX8Wrapper::Convert_Color(Color);

		for (i=0; i<8; i++)
		{			
			*(Vector3*)(vb_inner+fi.Get_Location_Offset())=vert[i];
			*(unsigned int*)(vb_inner+fi.Get_Diffuse_Offset())=color;
