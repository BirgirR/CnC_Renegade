	...
	*/
	...
	for (unsigned int chidx = 0; chidx < num_points - 1; chidx += (chunk_size - 1)) {
		...
		*/
		...
		Vector3 *points_inner = xformed_subdiv_pts;
		...
		*/
		...
		for (sidx = 1; sidx < point_cnt; sidx++) {	// #segments = #points - 1 (+ 2 dummy segments)
			...
			Vector3 &curr_point = points_inner[sidx - 1];
			Vector3 &next_point = points_inner[sidx];
			...
			if (sidx > 1) {
				...
				Vector3 prev_plane;
				Vector3::Cross_Product(points_inner[sidx - 2], curr_point, &prev_plane);
		...
		*/
		...
		intersection[1][TOP_EDGE].Point = points_inner[0];
		...
		intersection[1][BOTTOM_EDGE].Point = points_inner[0];
		...
		Vector3 &first_point = points_inner[0];
		...
		Vector3 segdir = points_inner[1] - points_inner[0];
		...
		intersection[last_isec][TOP_EDGE].Point = points_inner[point_cnt - 1];
		...
		intersection[last_isec][BOTTOM_EDGE].Point = points_inner[point_cnt - 1];
		...
		Vector3 &last_point = points_inner[point_cnt - 1];
		...
		segdir = points_inner[point_cnt - 1] - points_inner[point_cnt - 2];
		segdir.Normalize();	// Is this needed? Probably not - remove later when all works
		Vector3::Cross_Product(top, bottom, &start_pl);
		start_pl.Normalize();
		...
		for (iidx = 2; iidx < num_intersections[TOP_EDGE]; iidx++) {
			...
			// Relevant midpoint:
			Vector3 &midpoint = points_inner[iidx - 1];
...
// GENERALIZE FOR WHEN NO TEXTURE (DO NOT SET UV IN THESE CASES? NEED TO GENERALIZE FOR DIFFERENT TEXTURING MODES ANYWAY).
		...
		top = top_dir * Vector3::Dot_Product(points_inner[0], top_dir);
		Vector3 &bottom_dir = intersection[1][BOTTOM_EDGE].Direction;
		bottom = bottom_dir * Vector3::Dot_Product(points_inner[0], bottom_dir);
		vArray[vidx].x = top.X;
		vArray[vidx].y = top.Y;
		vArray[vidx].z = top.Z;
		...
		unsigned int delta_inner = MIN(residual_top_points, residual_bottom_points) - 1;
		residual_top_points -= delta_inner;
		residual_bottom_points -= delta_inner;
		pidx += delta_inner;
		...
		for (; ; ) {
			...
			if (residual_top_points == 1 && residual_bottom_points == 1) {
				...
				Vector3 &top_dir_inner = intersection[top_int_idx][TOP_EDGE].Direction;
				top = top_dir_inner * Vector3::Dot_Product(points_inner[pidx], top_dir_inner);
				Vector3 &bottom_dir_inner = intersection[bottom_int_idx][BOTTOM_EDGE].Direction;
				bottom = bottom_dir_inner * Vector3::Dot_Product(points_inner[pidx], bottom_dir_inner);
			...
			// Reduce both pointcounts by the same amount so the smaller one is 1 (skip points)
			delta_inner = MIN(residual_top_points, residual_bottom_points) - 1;
			residual_top_points -= delta_inner;
			residual_bottom_points -= delta_inner;
			pidx += delta_inner;
