...
static void Log_Textures(bool inited,unsigned& total_count, unsigned& total_mem)
{
	...
	for (ite.First();!ite.Is_Done();ite.Next()) {
		...
		WWDEBUG_SAY(("%32s	%4d * %4d (%15s), init %d, size: %14s bytes, refs: %d\n",
			(const char *)tex->Get_Texture_Name(),
			desc.Width,
			desc.Height,
			(const char *)tex_format,
			tex->Is_Initialized(),
			(const char *)number,
			tex->Num_Refs()));
...
HAnimClass *	WW3DAssetManager::Get_HAnim(const char * name)
{
	...
	// Special case the Null render object, the same way Find_Prototype does.
	// "NULL" is a reserved name for the built-in placeholder object; it has no
	// hierarchy, so "NULL.<anything>" can never name a real animation. Without
	// this we spend every call trying to load NULL.w3d, failing, and recording
	// it as a missing asset.
	if (name != NULL && strnicmp(name,"NULL.",5) == 0) {
		return NULL;
	}
...
void WW3DAssetManager::Log_All_Textures(void)
{
	...
	for (ite.First();!ite.Is_Done();ite.Next()) {
		...
		WWDEBUG_SAY(("%4.4dkb %s%s\n",bytes/1024,(const char *)tmp,(const char *)t->Get_Texture_Name()));
	}
