...
*/
inline bool IntersectionClass::Intersect_Polygon_Z(IntersectionResultClass *FinalResult, Vector3 &PolygonNormal, Vector3 &v1, Vector3 &v2, Vector3 &v3)
{

	FinalResult->Range = Plane_Z_Distance(PolygonNormal, v1);
	(FinalResult->Intersection)[0] = (*RayLocation)[0];
	(FinalResult->Intersection)[1] = (*RayLocation)[1];
	(FinalResult->Intersection)[2] = (*RayLocation)[2] - FinalResult->Range;
	return _Point_In_Polygon(FinalResult, PolygonNormal, v1, v2, v3);
}
...
*/
void IntersectionClass::Calculate_Intersection(IntersectionResultClass *FinalResult)
{
	(FinalResult->Intersection)[0] = (*RayLocation)[0] + (*RayDirection)[0] * FinalResult->Range;
	(FinalResult->Intersection)[1] = (*RayLocation)[1] + (*RayDirection)[1] * FinalResult->Range;
	(FinalResult->Intersection)[2] = (*RayLocation)[2] + (*RayDirection)[2] * FinalResult->Range;
}
...
*/
inline bool IntersectionClass::Intersect_Plane_Quick(IntersectionResultClass *FinalResult, Vector3 &PlaneNormal, Vector3 &PlanePoint) 
{
	// do a parallel check
	float divisor = (PlaneNormal[0] *(*RayDirection)[0] + PlaneNormal[1] *(*RayDirection)[1] + PlaneNormal[2] * (*RayDirection)[2]);
	if(divisor == 0) return false; // parallel
	...
	FinalResult->Range = - (d + PlaneNormal[0] * (*RayLocation)[0] + PlaneNormal[1] *  (*RayLocation)[1] + PlaneNormal[2] *  (*RayLocation)[2])  / divisor;
...
*/
inline bool IntersectionClass::Intersect_Plane(IntersectionResultClass *FinalResult, Vector3 &PlaneNormal, Vector3 &PlanePoint) {
								 
	// normalize the ray direction
	RayDirection->Normalize();

	// call the quick test routine
	if(!Intersect_Plane_Quick(FinalResult, PlaneNormal, PlanePoint)) return false;

	// check to make sure it's not behind the ray's origin
	if(FinalResult->Range <= 0) return false; 

	// check to make sure it's not beyond max distance
	if(FinalResult->Range > MaxDistance) return false;

	// determine point of intersection
	Calculate_Intersection(FinalResult);
...
*/
inline bool IntersectionClass::Intersect_Polygon(IntersectionResultClass *FinalResult, Vector3 &PolygonNormal, Vector3 &v1, Vector3 &v2, Vector3 &v3)
{
	...
	if(Intersect_Plane(FinalResult, PolygonNormal, v1)) {
...
*/
inline bool IntersectionClass::Intersect_Polygon(IntersectionResultClass *FinalResult, Vector3 &v1, Vector3 &v2, Vector3 &v3)
{
	Vector3 vec1 = v2 - v1;
	Vector3 vec2 = v3 - v1;
	Vector3 normal = Vector3::Cross_Product(vec1, vec2);

	return Intersect_Polygon(FinalResult, normal, v1,v2,v3);
}
...
bool IntersectionClass::Intersect_Screen_Object(	IntersectionResultClass *Final_Result,
											...
											RenderObjClass *obj)
{
	Final_Result->Intersects = ((ScreenX >= Area[0]) && (ScreenX <= Area[2]) &&
										(ScreenY >= Area[1]) && (ScreenY <= Area[3]));
